/**
 * @file <argos3/plugins/robots/drone/hardware/drone_rangefinders_default_sensor.cpp>
 *
...
namespace argos {
   ...
   CDroneRangefindersDefaultSensor::CDroneRangefindersDefaultSensor() {}
   ...
   void CDroneRangefindersDefaultSensor::Init(TConfigurationNode& t_tree) {
      try {
         ...
         if(strCalibrationFilePath.empty()) {
            LOGERR << "[WARNING] No calibration data provided for the drone rangefinders" << std::endl;
         }
         else {
            std::string strLabel;
            ...
            TConfigurationNode& tSensorNode = GetNode(tCalibrationNode, "drone_rangefinders");
            /* read the parameters */
